...
function generateMovieList(selectedList){
    ...
    db.collection(currentUserEmail).doc(selectedList).get().then((doc)=>{
        ...
        movieObjArray.forEach(movieObj =>{
            ...
            if (rating != ""){
                appendWatchedMovieList(title,rating);
            }
            else{
...
function populateListSelect(){
    ...
    deleteButton.classList = "list-delete-button btn-small red waves-effect";
    
    const addButton = document.createElement('button');
    addButton.textContent = "Add List";
    addButton.classList = "list-add-button btn-small blue waves-effect";
...
function appendUnwatchedMovieList(title){
    movieList.innerHTML += `<li data-movieUnwatched="${title}"> <div class="collapsible-header">${title} ${appendStreaming(pullMovieObjOut(title))}
    <select data-movie="${title}" id="ratings" class="browser-default rate">
    ...
    <div class="collapsible-body"><span>${appendInfo(pullMovieObjOut(title))}</span></div>
...
function appendWatchedMovieList(title, rating){
    watchedMovies.querySelector(`.${rating}-container`).innerHTML += `<div class="${rating} watched-movie" data-movieWatched="${title}">
                ${title} ${appendStreaming(movieObj)}<button id="unwatch" class="waves-effect waves-light btn-small"data-movie="${title}" >Unwatch/Rate</button></div>`;
}